...
const getDetails = (dom = document, link = window.location.href) => {
  ...
  if (info.magnets && info.magnets.length > 0) {
    ...
    info.magnets.forEach((magnet) => {
      ...
      } else if (!info.tagNames.ZH.length && zh && number <= 6) {
        info.tagNames.ZH2.push(nameTXT);
...
function displayTags(movie, tagNames) {
  ...
  createTag(tagsDiv, displayTagsName(tagNames.ZH2), tagNames.ZH2, 'is-warning2');
...
(function () {
  ...
  const autoRename = async () => {
    ...
    for (const item of items) {
      ...
      await handleRenameKey(hoverTarget, item_box); // 调用重命名函数
      // await handleRenameKey(hoverTarget, item_box, pid); // 调用重命名函数
      refreshMatchApi(item_box);
