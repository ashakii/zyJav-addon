...
const scriptKey = "javdb_script_enabled";
let enabled = GM_getValue(scriptKey, true);

function toggleScript() {
  enabled = !enabled;
  GM_setValue(scriptKey, enabled);
  if (!enabled) {
    console.log("JavDB 脚本已禁用");
    location.reload(); // 重新加载页面以禁用脚本
  } else {
    console.log("JavDB 脚本已启用");
...
if (!enabled) return; // 如果禁用，直接返回，不执行脚本后续代码
