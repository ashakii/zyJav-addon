...
(function () {
  const SELECTOR = ":is(.actors, .movie-list, .section-container, .zy-match) a:not(.button)";

  const click = (e) => {
    const target = e.target.closest(SELECTOR);
    if (!target || e.target.closest(".x-retag") || e.target.closest(".zy-match")) return;
