...
class Req115 extends Drive115 {
  ...
  static filesAllImgs(cid, params = {}) {
    return this.filesAll(cid, { ...params, type: 2 });
  }
  ...
  static async localVerify(cid, codes, filter) {
    let videos = [];
    const { data } = await this.filesAllVideos(cid);
    codes = codes.map((code) => code.toUpperCase());
    ...
    return { videos: videos.filter(filter) };
  }
  ...
  static handleRename(files, cid, { rename, renameTxt, zh, crack, wuma, fourk }) {
  ...
  static async handleCover(url, cid, filename) {
    const file = await this.request({
      url, timeout: 60000, responseType: "blob", headers: {
  ...
  static async handleOffline(
    ...
    for (let index = 0, { length } = magnets; index < length; index++) {
      ...
      if (rename) this.handleRename(files, file_id, { rename, renameTxt, zh: zh || srts.length, crack, fourk });
